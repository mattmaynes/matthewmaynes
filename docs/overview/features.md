...
- Previous/next post navigation (spec 0021): the bottom of each post links to its chronological
  ...
  and the post's tags as badges under the title - spec 0023). Adjacency is the pure
...
- Listing discovery (spec 0012, 0028): a `"use client"` island (`src/components/blog-list.tsx`) renders
  the rows and owns tag filtering - a single-select Canopy `Combobox` (spec 0028; an "All posts" entry
