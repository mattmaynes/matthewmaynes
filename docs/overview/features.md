...
| `/blog/[slug]` | ✅ | Individual post (MDX + frontmatter, statically generated): breadcrumb, header (title, byline + avatar, date + reading-time pill, a category badge + tag-archive links), cover hero (with a series sash when the post is in a series), the MDX body at an 18px measure with blur-placeholder inline images and self-hosted video, an optional author-placed mid-article subscribe aside (`<PostSubscribe />`, spec 0041), a disclaimer, a subscribe block, previous/next nav, and a "Back to blog" link. The cover doubles as the per-post OG/Twitter card. |
