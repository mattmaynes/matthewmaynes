...
- Pure, fs-free view helpers live in `blog-view.js` (the one `slugify`, `tagSlug`/`tagFromSlug`,
  ...
  `PostRowData`/`Cover` types) so BOTH the `"use client"` listing island and the Server
  Component pages import them. The listing row is a hook-free presentational `post-row.tsx` shared by
  the island, the tag archive, and the category archive; a server-only `post-summaries.ts` maps posts to row data (resolving
...
- **Drafts + scheduling:** a `draft: true` flag and an optional `publishAt` timestamp give each post
  ...
  `/subscribe`, feed, sitemap, tag + category pages, "New" badge, published nav + `generateStaticParams`) uses the
...
- **Structured data + AEO (spec 0040):** JSON-LD is built by pure, fs-free functions in
  ...
  (`src/components/json-ld.tsx`) that escapes `<` as `<`, so the five node types can't drift in
  their escaping. `Person` carries `worksFor` (the current role from `resume.work`, `period` ~
  `/current/i`), `description`, and `knowsAbout` (`resume.skills`). `/` emits `WebSite`, `/blog`
  emits `Blog`, and each **published** post emits `BlogPosting` + `BreadcrumbList` behind the same
  `isPublishedNow` guard as the page/OG route (so nothing leaks pre-`publishAt`). A build-generated
  **`/llms.txt`** (`src/lib/llms.ts` pure builder + `app/llms.txt/route.ts` thin shell, `revalidate =
  60`, `text/plain`) gives AI crawlers a curated markdown map of the site + every published post.
- **One source of truth:** identity/social/`nav` come from `src/lib/site.ts`; the shared TEXT
  constants (`tagline`, `description`, `ogImageAlt`, `blogFeedTitle`) live in the asset-free
  `src/lib/site-text.ts` (split from `site.ts`, which static-imports images Node's test runner can't
  load, so the pure JSON-LD/`llms` builders read text there; `site.ts` re-exports them - still one
  source). The sitemap enumerates nav routes + every post + every tag and category archive. The
  prose-word normalizer is the one `blog-view.stripToProse` (shared by `estimateReadingMinutes` and
  the `BlogPosting` `wordCount`). Icons are generated by `scripts/build-icons.ts` (`qlmanage` + `sips` + a stdlib
