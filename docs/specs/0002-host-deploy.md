# 0002 - Server deploy (matthewmaynes.com)
...
managed HTTPS. The server is also intended to **cohost** future sites/containers, so the setup
...
- The server runs two stacks on a shared `edge` network: a **Caddy** reverse proxy (owns 80/443)
...
- The production image is pulled from **GHCR** (`ghcr.io/mattmaynes/matthewmaynes`), built
  off-host so the 512MB server never runs a memory-hungry Next build.
- **Pushing to `main` ships it.** A GitHub Actions pipeline builds the image, pushes it to GHCR,
  and triggers the server to pull and restart - no manual step in the normal path.
...
- **GitHub Actions CD pipeline** (`.github/workflows/deploy.yml`): on push to `main`, build
  `linux/amd64`, push to GHCR, then SSH to the server to pull + restart the site stack.
- Manual build/push + server pull documented as the **bootstrap/fallback** path (first image
  before the server exists; recovery if the pipeline is down).
- DNS records (apex + `www`) at the registrar.
- Operator runbook (kept privately, git-ignored - not in the public repo): create the VM, swap,
  install Docker, create `edge` network, firewall, deploy SSH key, bring up proxy + site,
  first-deploy + update.
- Make the GHCR package **public** so the server pulls without registry auth.
...
- `Caddyfile`: `matthewmaynes.com` block → `reverse_proxy site:3000`, with `encode zstd gzip`.
  ...
  Let's Encrypt certs (needs DNS pointing at the server first). HTTP->HTTPS is automatic.
...
- **Deploy job** (after build): SSH to the server and, in the cloned repo, run `git pull` then
  `docker compose -f deploy/docker/compose.site.yml pull && ... up -d`. Uses repo secrets
  `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY` (a dedicated deploy key, not a personal key).
  The server pulls the **public** GHCR image, so it needs no registry login.
...
- First image, before the server or pipeline exists - or recovery if Actions is down:
  `docker buildx build --platform linux/amd64 -t ghcr.io/mattmaynes/matthewmaynes:latest --push .`
  (authenticate once with a GHCR PAT carrying `write:packages`), then on the server
...
- Bootstrap order: push the first image (manual or a first pipeline run) -> provision server + DNS
  -> proxy + site up -> thereafter every merge to `main` auto-deploys.

**DNS (registrar).**
- In the registrar's DNS settings, remove any default parking/redirect records, then add:
  `A  @   -> <server-ipv4>` and `A  www -> <server-ipv4>` (and optional `AAAA` if IPv6 enabled).
- TTL low (e.g. 5 min) during cutover. Caddy only obtains certs once these resolve to the server.

**Server provisioning (operator runbook, kept privately).**
- Ubuntu LTS VM, about **512MB/1vCPU**. Build is off-host, so runtime fits;
...
- [ ] The server runs the site from the GHCR image (no `build` step on the host).
- [ ] A push to `main` triggers the pipeline: image built, pushed to GHCR (tagged `latest` + `sha`),
      and the server auto-pulls and restarts with the new image (verified by a visible change or
...
- [ ] The operator runbook reproduces the deploy from a fresh VM.
