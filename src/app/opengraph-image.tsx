...
// Fonts: Figtree TTFs colocated in _og/ (satori cannot read the woff2 that
// @fontsource ships). `new URL(..., import.meta.url)` makes them traced assets
// so they survive the standalone build. The logo reads from public/, which the
// Dockerfile/standalone copy step deploys alongside server.js.
async function loadAssets() {
  const [regular, semibold, bold, logo] = await Promise.all([
    readFile(new URL("./_og/figtree-400.ttf", import.meta.url)),
    readFile(new URL("./_og/figtree-600.ttf", import.meta.url)),
    readFile(new URL("./_og/figtree-700.ttf", import.meta.url)),
